...
VSCPRead(long handle, vscpEvent* pEvent, unsigned long timeout)
{
    ...
    if (-1 == (rv = sem_timedwait(&pdrvObj->m_semReceiveQueue, &ts))) {
        if (ETIMEDOUT == errno) {
            ...
            syslog(LOG_ERR,
                   "[vscpl2drv-table] Interrupted by a signal handler");
            return CANAL_ERROR_INTERNAL;
        } else if (EINVAL == errno) {
            syslog(LOG_ERR, "[vscpl2drv-table] Invalid semaphore (timout)");
            return CANAL_ERROR_INTERNAL;
        } else if (EAGAIN == errno) {
            syslog(LOG_ERR, "[vscpl2drv-table] Blocking error");
            return CANAL_ERROR_INTERNAL;
        } else {
            syslog(LOG_ERR, "[vscpl2drv-table] Unknown error");
            return CANAL_ERROR_INTERNAL;
        }
    }
