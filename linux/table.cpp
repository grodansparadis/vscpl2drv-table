...
CTcpipSrv::open(std::string& path, const cguid& guid)
{
    ...
    if (!doLoadConfig()) {
        syslog(LOG_ERR,
               "[vscpl2drv-table] Failed to load configuration file [%s]",
               path.c_str());
        return false;
    }
    ...
    if (!startTcpipSrvThread()) {
        syslog(LOG_ERR,
               "[vscpl2drv-table] Failed to start server.");
        return false;
    }
...
CTcpipSrv::doLoadConfig(void)
{
    try {
        ...
        syslog(LOG_ERR, "[vscpl2drv-table] Failed to parse JSON configuration.");
        return false;
    }

    if (!readEncryptionKey(m_j_config.value("vscp-key-file", ""))) {
        syslog(LOG_ERR, "[vscpl2drv-table] WARNING!!! Default key will be used.");
    ...
    for (json::iterator it = m_j_config["users"].begin(); it != m_j_config["users"].end(); ++it) {
        ...
        if (!m_userList.addUser((*it).value("name", ""),
                                    ...
                                    (*it).value("flags", 0))) {
            syslog(LOG_ERR, "[vscpl2drv-table] Failed to add client %s.",(*it).dump().c_str());
        }
    }
...
CTcpipSrv::handleHLO(vscpEvent* pEvent)
{
    ...
    if (NULL == pEvent || (NULL == pEvent->pdata)) {
        syslog(LOG_ERR,
               "[vscpl2drv-table] HLO handler: NULL event pointer.");
        return false;
    }
    ...
    //     syslog(LOG_ERR, "[vscpl2drv-table] Failed to parse HLO.");
    //     return false;
    // }
    ...
    if (!j["op"].is_string() || 
             j["op"].is_null()) {
        syslog(LOG_ERR,"[vscpl2drv-table] HLO-command: Missing op [%s]",j.dump().c_str());         
        return false;
    }
...
CTcpipSrv::readVariable(vscpEventEx& ex, const json& json_req)
{
    ...
    if ("enable-debug" == j.value("name","")) {
        ...
        if (!m_j_config["users"].is_array()) {
            syslog(LOG_WARNING,"[vscpl2drv-table] 'users' must be of type array.");
            j["result"] = ERR_VARIABLE_OK;
            goto abort;
        }
        ...
        if (index >= m_j_config["users"].size()) {
            // Index to large
            syslog(LOG_WARNING,"[vscpl2drv-table] index of array is to large [%u].", 
...
CTcpipSrv::writeVariable(vscpEventEx& ex, const json& json_req)
{
    ...
    if ("enable-debug" == j.value("name","")) {
        ...
        if (!m_j_config["args"].is_object()) {
            syslog(LOG_WARNING,"[vscpl2drv-table] The user info must be an object.");
            j["result"] = ERR_VARIABLE_WRONG_TYPE;
            goto abort;
        }
        ...
        if (index >= m_j_config["users"].size()) {
            // Index to large
            syslog(LOG_WARNING, 
                        "[vscpl2drv-table] index of array is to large [%u].",
                        index >= m_j_config["users"].size());
            j["result"] = ERR_VARIABLE_INDEX_OOB;
            goto abort;
        }
...
CTcpipSrv::deleteVariable(vscpEventEx& ex, const json& json_reg)
{
    ...
    if ("users" == j.value("name","")) {
        ...
        j["result"] = ERR_VARIABLE_UNKNOWN;
        syslog(LOG_WARNING, "[vscpl2drv-table] Variable [%s] is unknown.", j.value("name", "").c_str());
    }
...
{
    if (!stop()) {
        syslog(LOG_WARNING, "[vscpl2drv-table] Failed to stop VSCP tcp/ip server.");
    }

    if (!start()) {
        syslog(LOG_WARNING, "[vscpl2drv-table] Failed to start VSCP tcp/ip server.");
    }
...
CTcpipSrv::eventExToReceiveQueue(vscpEventEx& ex)
{
    ...
    if (!vscp_convertEventExToEvent(pev, &ex)) {
        syslog(LOG_ERR,
               "[vscpl2drv-table] Failed to convert event from ex to ev.");
    ...
    if (NULL != pev) {
        ...
        syslog(LOG_ERR,
               "[vscpl2drv-table] Unable to allocate event storage.");
    }
    return true;
}
...
CTcpipSrv::readEncryptionKey(const std::string& path)
{
    try {
        ...
        syslog(LOG_ERR,
                "[vscpl2drv-table] Failed to read encryption key file [%s]",
                m_path.c_str());
        return false;
    }
